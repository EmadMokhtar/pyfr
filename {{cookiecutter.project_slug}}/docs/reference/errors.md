...
covers:
  - src/{{ cookiecutter.package_name }}/api/errors.py
---
...
{%- else %}
| 503 | `storage_unavailable` | Published by the contract for `GET /api/v1/orders/{order_id}/receipt`, so a client written against it handles the case; the in-memory receipt store this service runs on never raises it. |
{%- endif %}
