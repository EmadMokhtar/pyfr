...
| Contract | Forbids |
| --- | --- |
{%- set _forbidden = ["FastAPI", "Starlette"] + (["SQLAlchemy", "asyncpg"] if cookiecutter.database == "postgres" else []) + ["OpenTelemetry", "httpx", "stamina"] + (["redis"] if cookiecutter.cache == "redis" else []) + (["aioboto3"] if cookiecutter.object_storage == "s3" else []) %}
| `domain-independence` | `domain` importing `api`, `services`, `infrastructure`, the container, the app factory, or any of: {{ _forbidden | join(', ') }} |
...
{%- else %}
no other adapters satisfy — and it holds *another* `OrderRepository` inside
{%- endif %}
it:
