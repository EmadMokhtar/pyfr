...
def test_every_render_carries_the_weekly_template_update(cookies, answers) -> None:
    ...
    assert "release_url=$(jq -r '.release_url // empty' check.json)" in workflow
    assert "${TEMPLATE}/releases/tag/" not in workflow
    # #55: a refused `gh pr create` or `gh issue create` names the missing
    # permission or the Actions setting, as the refused push already does
    # -- one annotation per permission the README says the token needs.
    assert "Resource not accessible" in workflow
    assert "not permitted to create or approve pull requests" in workflow
    assert "Write access to repository not granted" in workflow
    assert "lacks the Pull requests permission" in workflow
    assert "lacks the Issues permission" in workflow
    assert "lacks the Contents permission" in workflow
