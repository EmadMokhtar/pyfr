...
| D5 | Light DDD, four layers | Domain, services, infrastructure, api. Business logic testable with no I/O |
...
| `logger` | standard library logger name | `my_service.services.order` |
...
```
tests/
  unit/          domain + services. No I/O, no containers. Milliseconds.
...
`domain/` and `services/` (mutating adapters produces noise about error paths
...
types        mypy: strict on domain/ and services/, lenient elsewhere
...
services, where the logic lives and types encode business rules; lenient in
