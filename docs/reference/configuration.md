...
| `APP_PAYMENT__BASE_URL` | URL | unset, required once any `APP_PAYMENT__*` is set | The payment provider's base URL. **Leave it unset to run on the in-memory gateway, which authorises everything** — the same arrangement `APP_DATABASE__DSN` has with the in-memory repository. `just up` points it at a local stub. |
